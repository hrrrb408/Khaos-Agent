...
pub struct ToolCall {
    ...
    /// handlers were removed.  They performed raw filesystem and process
    /// operations with no path authorization, no sandbox, and no resource
    /// limits — a single ``run_parallel_json`` call from Python (which IS
    /// reachable via the PyO3 module) could read ``/etc/shadow``, write
    /// arbitrary host paths, or spawn arbitrary processes, bypassing the
    /// entire Python security stack (Sandbox / PathGuard / CommandGuard /
    /// PermissionEngine).  They were never wired into production tool
    /// dispatch, so removing them closes the bypass without losing
    /// functionality.  Token counting (the only production Rust user) is
    /// unaffected.
    pub kind: String,
...
async fn dispatch(call: &ToolCall) -> Result<String, ExecutorError> {
    match call.kind.as_str() {
        ...
        // Round-14 §6: read_file / write_file / exec handlers removed.
        // See the ``ToolCall::kind`` doc comment for the security rationale.
        // Unknown kinds (including the removed ones) report an error.
...
mod tests {
    ...
    // --- removed I/O/exec handlers (Round-14 §6) --------------------------
    //
    // The ``read_file`` / ``write_file`` / ``exec`` handlers were removed
    // because they bypassed the Python security stack entirely.  These
    // regression tests pin that they are now reported as unknown kinds, so a
    // future change cannot silently reintroduce the bypass: any payload
    // requesting those kinds must fail closed.

    #[test]
    fn removed_file_and_exec_handlers_fail_closed() {
        let rt = runtime();
        for kind in ["read_file", "write_file", "exec"] {
            let results = rt.block_on(run_parallel(
                vec![call("1", kind, r#"{"path":"/etc/shadow"}"#)],
                500,
            ));
            assert!(!results[0].success, "{kind} should not be a known handler");
            assert!(
                results[0].error.contains("unknown handler kind"),
                "{kind} error was: {}",
                results[0].error
            );
        }
