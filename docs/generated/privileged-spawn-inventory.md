...
- `python/khaos/coding/execution/native_launcher_runtime.py:387` `subprocess.run` in `<module>._authority_path` owner=`NativeLauncherTCB` threat-model=`fd-bound-exec-and-codesign` boundary=`native-launcher`
...
- `python/khaos/security/production_composition_probe.py:107` `subprocess.Popen` in `<module>._spawn_probe_process` owner=`ProductionCompositionProbe` threat-model=`compose-job-identity` boundary=`production-compose-e2e`
