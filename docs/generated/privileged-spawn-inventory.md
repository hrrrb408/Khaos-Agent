...
- `python/khaos/cli/main.py:498` `subprocess.run` in `<module>.cmd_start` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
- `python/khaos/cli/main.py:512` `subprocess.Popen` in `<module>.cmd_start` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
- `python/khaos/cli/main.py:573` `subprocess.run` in `<module>.cmd_test` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
- `python/khaos/cli/main.py:583` `subprocess.run` in `<module>.cmd_test` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
...
- `python/khaos/coding/execution/native_launcher_runtime.py:131` `os.execve` in `<module>.main` owner=`NativeLauncherTCB` threat-model=`fd-bound-exec-and-codesign` boundary=`native-launcher`
- `python/khaos/coding/execution/native_launcher_runtime.py:144` `os.execve` in `<module>.main` owner=`NativeLauncherTCB` threat-model=`fd-bound-exec-and-codesign` boundary=`native-launcher`
- `python/khaos/coding/execution/native_launcher_runtime.py:150` `os.execvpe` in `<module>.main` owner=`NativeLauncherTCB` threat-model=`fd-bound-exec-and-codesign` boundary=`native-launcher`
- `python/khaos/coding/execution/native_launcher_runtime.py:408` `subprocess.run` in `<module>._authority_path` owner=`NativeLauncherTCB` threat-model=`fd-bound-exec-and-codesign` boundary=`native-launcher`
...
- `python/khaos/coding/workspace/trusted_git.py:2337` `subprocess.Popen` in `<module>._run_sync_bounded` owner=`TrustedGitRunner` threat-model=`untrusted-repository-config` boundary=`workspace-control-plane`
- `python/khaos/evaluation/coding/fixtures.py:296` `asyncio.create_subprocess_exec` in `<module>._git_output` owner=`FixtureManager` threat-model=`private-fixture-git` boundary=`coding-evaluation-fixture`
