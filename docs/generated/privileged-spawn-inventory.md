...
- `python/khaos/cli/main.py:425` `subprocess.run` in `<module>.cmd_test` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
- `python/khaos/cli/main.py:435` `subprocess.run` in `<module>.cmd_test` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
...
- `python/khaos/coding/execution/supervisor.py:1599` `subprocess.run` in `<module>._darwin_deleted_open_file_usage` owner=`ProcessSupervisor` threat-model=`child-tree-lifecycle` boundary=`execution-service`
...
- `rust/khaos-core/src/bin/khaos-sandbox-launcher.rs:577` `Command::new` in `rust::entrypoint` owner=`LinuxSandboxLauncher` threat-model=`seccomp-landlock-bwrap-boundary` boundary=`linux-sandbox`
