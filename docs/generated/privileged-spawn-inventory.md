...
- `python/khaos/cli/main.py:405` `subprocess.run` in `<module>.cmd_start` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
- `python/khaos/cli/main.py:419` `subprocess.Popen` in `<module>.cmd_start` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
- `python/khaos/cli/main.py:480` `subprocess.run` in `<module>.cmd_test` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
- `python/khaos/cli/main.py:490` `subprocess.run` in `<module>.cmd_test` owner=`CliGatewayProcess` threat-model=`user-authorized-local-gateway` boundary=`cli-runtime`
