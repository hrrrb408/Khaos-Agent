...
- `python/khaos/coding/workspace/trusted_git.py:2343` `subprocess.Popen` in `<module>._run_sync_bounded` owner=`TrustedGitRunner` threat-model=`untrusted-repository-config` boundary=`workspace-control-plane`
- `python/khaos/evaluation/coding/benchmark.py:322` `subprocess.run` in `<module>.capture_working_tree_identity` owner=`QualificationProvenance` threat-model=`trusted-source-git-provenance` boundary=`qualification-jsonl`
- `python/khaos/evaluation/coding/benchmark.py:330` `subprocess.run` in `<module>.capture_working_tree_identity` owner=`QualificationProvenance` threat-model=`trusted-source-git-provenance` boundary=`qualification-jsonl`
