...
- Windows native helper 已实现 native 与 trusted Python `network=none` 的 AppContainer/no-network、trusted Python direct base-executable + child-process policy、Job Object 单 native runtime 上限、workspace ACL、精确 runtime-root/file ACL 与 WFP Firewall contract；brokered execution 使用 restricted-token + exact loopback WFP；外层 helper 负责等待与清理；真实执行能力仍以当前 commit 的 Windows hosted probe/执行 job 为准。
