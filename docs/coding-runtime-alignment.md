...
- Windows native helper 已实现 restricted token、Job Object 单 native runtime 上限、workspace ACL 与 WFP Firewall contract；外层 helper 负责等待与清理；真实执行能力仍以当前 commit 的 Windows hosted probe/执行 job 为准。
