...
| `Platform Sandbox Security E2E / windows-fail-closed-security` | Windows hosted runner | 构建并 probe native helper；restricted token、Job Object 两进程上限树、workspace ACL、runtime read/execute ACL、WFP Firewall；native execution 与 no-Host-fallback contract | Windows AppContainer/private-desktop parity、Linux namespace/browser kernel isolation |
