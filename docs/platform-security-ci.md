...
| `Platform Sandbox Security E2E / windows-fail-closed-security` | Windows hosted runner | 构建并 probe native helper；native 与 trusted Python `network=none` 的 AppContainer token/no-network、trusted Python staged base-executable launch、brokered restricted-token + exact loopback WFP、Job Object 单 native runtime 上限、workspace ACL、一次性 runtime staging ACL、WFP Firewall；native execution 与 no-Host-fallback contract | Windows private-desktop parity、Linux namespace/browser kernel isolation |
...
`network=none` 路径对 native commands 与 trusted Python 验证
AppContainer/no-network contract；trusted Python 还验证 staged base-executable
launch、child-process policy、一次性 runtime staging ACL contract，brokered path 验证
restricted-token 与 exact loopback WFP contract；但不宣称 Windows private desktop 或浏览器专用 Linux netns parity。TCB 仅为完整性 SACL
