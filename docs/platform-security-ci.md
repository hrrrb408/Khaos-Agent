...
| --- | --- | --- | --- |
| `Security Contract Matrix / contract` | Ubuntu 24.04、Windows 2025、macOS 14 | Approval、turn、permission profile、durable webhook replay、Gateway/RPC、managed resource、TUI、Go/Rust；POSIX 额外验证 Verification Authority 与 dirfd workspace boundary | 真实 OS 隔离 |
| `Platform Sandbox Security E2E / linux-bwrap-security` | Ubuntu hosted runner | bwrap workspace-write/read-only、禁网、secret root、PID/process tree、HOME/TMP size 与 entry budget、无 Host fallback | Windows sandbox |
| `Platform Sandbox Security E2E / macos-sandbox-security` | macOS hosted runner | sandbox-exec workspace-write、外部写拒绝、禁网、secret root、pasteboard/Keychain IPC、whole-HOME budget 拒绝 | Linux namespace |
