...
| `Platform Sandbox Security E2E / windows-fail-closed-security` | Windows hosted runner | 构建并 probe native helper；native 与 trusted Python `network=none` 的 AppContainer token/no-network、trusted Python direct base-executable launch、brokered restricted-token + exact loopback WFP、Job Object 单 native runtime 上限、workspace ACL、精确 runtime-root/file ACL、WFP Firewall；native execution 与 no-Host-fallback contract | Windows private-desktop parity、Linux namespace/browser kernel isolation |
...
AppContainer/no-network contract；trusted Python 还验证 direct base-executable
launch、child-process policy、runtime-root/file ACL contract，brokered path 验证
