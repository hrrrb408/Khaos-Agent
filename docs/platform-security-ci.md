...
| `Platform Sandbox Security E2E / windows-fail-closed-security` | Windows hosted runner | 构建并 probe native helper；`network=none` 的 AppContainer token/no-network、restricted token、Job Object 单 native runtime 上限、workspace ACL、可信 venv/base-runtime read/execute ACL、WFP Firewall；native execution 与 no-Host-fallback contract | Windows private-desktop parity、Linux namespace/browser kernel isolation |
