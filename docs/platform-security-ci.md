...
| `Platform Sandbox Security E2E / windows-fail-closed-security` | Windows hosted runner | 构建并 probe native helper；native `network=none` 命令的 AppContainer token/no-network、trusted Python 的 restricted-token + WFP image block、Job Object 单 native runtime 上限、workspace ACL、精确 runtime-file ACL、WFP Firewall；native execution 与 no-Host-fallback contract | Windows private-desktop parity、Linux namespace/browser kernel isolation |
...
`network=none` 路径对 native commands 验证 AppContainer/no-network contract，trusted
Python path 验证 restricted-token、child-process policy、exact WFP image block 与
runtime-file ACL contract；但不宣称 Windows private desktop 或浏览器专用 Linux netns parity。TCB 仅为完整性 SACL
