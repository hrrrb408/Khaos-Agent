...
| Ordinary approval | `python/khaos/agent/approval.py`、permissions/、`tools/approval_callback.py` | broker/durable consume path；`ApprovalCallbackRunner` 只拥有 adapter 生命周期 | Gateway confirm、TUI dialog | one-shot、principal/context/args/expiry 绑定；回调线程未终止时不得报告 CLOSED |
...
| `python/khaos/tools/scheduler.py` | admission 后的 approval、authority、idempotency、并发和效果编排 | `ToolAdmission`、`ToolResultCodec`（结果归一化/持久化协议）、`ToolResultStore`（runtime replay cache）、`ApprovalCallbackRunner`（adapter 生命周期）；后续 `ToolAuthorization`、`ToolExecutionCoordinator` |
