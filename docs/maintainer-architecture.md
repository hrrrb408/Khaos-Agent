...
| `python/khaos/grpc_server.py` | transport/auth/startup、Agent/Memory/Session/Audit/Task service | protocol/auth middleware、composition root、每个 service 独立模块；服务只消费已认证 context |
...
- 从 `grpc_server.py` 提取 protocol/auth/service；保留兼容 adapter，不增加第二套 server authority。
