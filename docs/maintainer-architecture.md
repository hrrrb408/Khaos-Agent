...
| `python/khaos/tools/scheduler.py` | admission 后的 approval、authority、idempotency、并发和效果编排 | `ToolAdmission`、`ToolResultCodec`（结果归一化/持久化协议）、`ToolResultStore`（runtime replay cache）；后续 `ToolAuthorization`、`ToolExecutionCoordinator` |
