...
| Windows | native helper after a passing probe: native `network=none` commands use an OS-issued AppContainer low-box; the trusted Python interpreter uses a restricted primary token with child-process policy, exact WFP image block, exact runtime-file ACLs, one-process Job Object, and transactional workspace ACL; brokered mode is exact loopback-only | missing helper/probe evidence, non-native command, ACL/firewall/token/AppContainer failure, or unsupported network endpoint rejects execution; never falls back to Host |
