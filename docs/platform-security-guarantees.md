...
| Windows | native helper after a passing probe: restricted primary token, Job Object with kill-on-close/resource limits and one native runtime process, transactional workspace ACL, and WFP-backed Windows Firewall policy | missing helper/probe evidence, non-native command, ACL/firewall failure, or unsupported network endpoint rejects execution; never falls back to Host |
