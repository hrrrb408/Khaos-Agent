...
| Windows | native helper after a passing probe: `network=none` OS-issued AppContainer low-box with no network capability, one-process Job Object with kill-on-close/resource limits, transactional workspace ACL, and WFP-backed Windows Firewall policy; brokered mode separately uses a restricted primary token and is exact loopback-only | missing helper/probe evidence, non-native command, ACL/firewall/AppContainer failure, or unsupported network endpoint rejects execution; never falls back to Host |
