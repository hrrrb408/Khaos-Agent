...
- Generic webhook 必须配置至少 32 字符 secret，并验证 timestamp、one-shot message ID、
  platform、Khaos channel ID、body SHA-256 与 `X-Khaos-Signature` HMAC。
- Discord、Slack、WeChat 与 generic 必须验证签名时间窗；Telegram `update_id`、Discord
  interaction ID、Slack event/signature、WeChat `MsgId` 与 generic message ID 必须通过
