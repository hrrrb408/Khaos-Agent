...
| shell execution | argv/cwd/env/request policy 进入 exec runtime；审批判定与 OS sandbox 分离；process tree/输出有专用组件 | `PermissionProfile`、共享 supervisor、平台 selector 与 native helper 已接入；Host 仅保留为显式受限兼容组件，selector 不作安全 fallback | `tools/runtimes/shell.rs`, `exec-server`; `execution/service.py`, `platform.py`, `host.py` |
...
| Sandbox | ReadOnly/WorkspaceWrite/FullAccess/External 与 network policy；Seatbelt、Landlock/bwrap/seccomp、Windows restricted token/WFP | versioned immutable `PermissionProfile`；Linux/macOS/Windows backends 只在 probe 证明后执行，缺能力 fail closed | `protocol.rs`, `sandboxing/`; `security/sandbox.py`, `execution/models.py`, `execution/platform.py` |
...
| 网络策略 | request profile；Seatbelt/namespace/WFP；可配置 restricted/enabled 和 proxy | `NetworkBroker` 已提供 DNS/pinning/allowlist/audit/quotas；`NetworkLease` 绑定 capability；Linux brokered 只接受真实 namespace join，Host 不承载网络 policy | `permissions.rs`, platform sandbox; `security/network_broker.py`, `execution/models.py`, backends |
...
Khaos 现在把工具风险分类与执行权威分开：`PermissionProfile` 绑定 filesystem、network、roots、环境和资源，selector 只接受 probe 成功的 OS backend；Host 不再作为 selector 的安全 fallback。`NetworkBroker` 还把允许的外连收敛到 capability-bound lease，Linux 普通 loopback lease 会因 namespace 不可达而拒绝。结论：OS 强制仍以当前平台 backend 的真实 probe/CI 证据为准，但三条原先的本地执行缺口已经有明确的拒绝路径和实现。
