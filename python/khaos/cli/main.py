...
async def run_once(args: argparse.Namespace) -> int:
    ...
    from khaos.runtime import ProductionRuntimeConfig, build_runtime, close_runtime_or_register
    runtime = None
    try:
        runtime = await build_runtime(ProductionRuntimeConfig(db=db, mode_manager=mode_manager, confirm_callback=_confirm_from_args(args), principal_id=f"local-uid:{os.getuid()}", source_transport="cli", foreground_session=True, project_id=cli_project_id))
...
async def run_repl(args: argparse.Namespace) -> int:
    ...
    runtime = None
    try:
        runtime = await build_runtime(ProductionRuntimeConfig(db=db, mode_manager=mode_manager, confirm_callback=_interactive_confirm(args), principal_id=f"local-uid:{os.getuid()}", source_transport="cli", foreground_session=True, project_id=cli_project_id))
        loop = runtime.loop
