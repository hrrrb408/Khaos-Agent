...
class AgentService:
    ...
    def __init__(self, db: Database, project_root: Path | None = None):
        self.db = db
    ...
    async def _build_runtime(self, session_id: str, mode: str) -> tuple[ModeManager, AgentLoop]:
        ...
        loop = AgentLoop(
            ...
            confirm_callback=lambda request: self.pending_confirmations.pop(
                request["id"], {"approved": True, "remember": False}
            ),
...
async def serve_json_lines(host: str, port: int, db_path: str, project_root: Path | None = None) -> None:
    """Serve JSON-line RPC requests over TCP."""
    db = Database(db_path)
    await db.connect()
    await db.run_migrations()
    agent = AgentService(db, project_root=project_root)
...
def main() -> None:
    ...
    asyncio.run(serve_json_lines(args.host, args.port, args.db))
