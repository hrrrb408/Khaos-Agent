...
async def test_read_file_paginates_with_line_numbers(tmp_path):
    ...
    result = await read_file(str(file_path), offset=2, limit=2)
...
async def test_write_file_creates_parent_and_overwrites(tmp_path):
    ...
    result = await write_file(str(file_path), "hello")
...
async def test_patch_exact_replace(tmp_path):
    ...
    result = await patch(str(file_path), "beta", "gamma")
...
async def test_patch_fuzzy_replace(tmp_path):
    ...
    result = await patch(str(file_path), "color blue", "color green", fuzzy=True)
...
async def test_patch_raises_when_missing(tmp_path):
    ...
    try:
        await patch(str(file_path), "missing", "new", fuzzy=False)
...
async def test_search_files_by_glob_and_query(tmp_path):
    ...
    result = await search_files(str(tmp_path), query="a", glob="*.py", content=False)
...
async def test_search_files_by_content(tmp_path):
    ...
    result = await search_files(str(tmp_path), query="needle", glob="*.py", content=True)
