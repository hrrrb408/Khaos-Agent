"""Tests for capability-injected orchestrator tools."""
...
def test_orchestrator_module_has_no_mutable_authority_holder():
    from khaos.tools import orchestrator_tools

    assert not hasattr(orchestrator_tools, "_spawner")
    assert not hasattr(orchestrator_tools, "_runner")
    assert not hasattr(orchestrator_tools, "init_orchestrator")
...
async def test_spawn_subagent_success():
    ...
    result = await spawn_subagent(
        ...
        principal_id="user1", subagent_spawner=spawner,
    )
...
async def test_spawn_subagent_defaults():
    spawned = _StubTask(task_id="t1")
    spawner = _make_spawner(spawn_side_effect=lambda _task: spawned)

    result = await spawn_subagent(
        goal="only goal", principal_id="user1", subagent_spawner=spawner
    )
...
async def test_collect_results_not_initialized():
    result = await collect_results()
    assert result == {"ok": False, "error": "Orchestrator not initialized"}


async def test_collect_results_handles_timeout():
...
async def test_execute_plan_success():
    ...
    result = await execute_plan(
        plan_json, principal_id="user1", subagent_spawner=spawner
    )
...
async def test_execute_plan_invalid_json():
    spawner = _make_spawner()

    result = await execute_plan(
        "not valid json", principal_id="user1", subagent_spawner=spawner
    )
    assert result["ok"] is False
    assert "error" in result


async def test_execute_plan_empty_object_json():
    """A valid JSON object with no tasks parses but yields zero results."""
    spawner = _make_spawner()

    result = await execute_plan(
        '{"tasks": []}', principal_id="user1", subagent_spawner=spawner
    )
    assert result["ok"] is True
    assert result["total"] == 0


async def test_execute_plan_not_initialized():
...
async def test_subagent_status_returns_stats():
...
async def test_subagent_status_not_initialized():
    result = await subagent_status()
    assert result == {"ok": False, "error": "Orchestrator not initialized"}
...
async def test_broker_injects_principal_id_for_subagent_spawn():
...
async def test_build_subagent_service_exposes_owned_spawner(tmp_path):
    ...
    try:
        ...
        assert isinstance(service.spawner, SubAgentSpawner)
        assert isinstance(service.runner, SubAgentRunner)
    finally:
...
async def test_build_subagent_service_orchestrator_tools_not_initialized_after(
    ...
    try:
        ...
        result = await spawn_subagent(
            goal="x",
            principal_id="user1",
            subagent_spawner=service.spawner,
        )
...
async def test_spawn_subagent_shutdown_race_returns_ok_false() -> None:
...
async def test_orchestrator_tools_refuse_empty_principal(
    ...
    result = await handler(subagent_spawner=spawner, **kwargs)
    assert result["ok"] is False, (
    ...
    result = await handler(
        principal_id="", subagent_spawner=spawner, **kwargs
    )
    assert result["ok"] is False
    assert result["error"] == "principal_id is required"
