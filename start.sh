...
# start.sh does not clear or sandbox model-provider variables. The privileged
# Python-to-Gateway RPC capability is generated by the Python parent and passed
# to its compiled Gateway child through an inherited pipe FD, never an env var.
...
PYTHONPATH=python "$PYTHON_BIN" -m khaos.cli.main start \
    --socket "$ROOT/.cache/khaos-agent.sock" --db khaos.db --config config.yaml --gateway \
